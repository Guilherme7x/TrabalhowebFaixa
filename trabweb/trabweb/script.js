function submitOrder() {
    // Obter as quantidades dos produtos
    ...
    // Obter as opções de personalização
    const personalized1 = document.querySelector('input[name="personalized1"]:checked') ? document.querySelector('input[name="personalized1"]:checked').value : "Não";
    const personalized2 = document.querySelector('input[name="personalized2"]:checked') ? document.querySelector('input[name="personalized2"]:checked').value : "Não";

    // Montar a mensagem
    let message = "Olá, gostaria de fazer um pedido:\n";
    if (quantity1 > 0) {
        message += `- Produto: Faixa Parachoque 3M\n  Quantidade: ${quantity1}\n  Personalizada: ${personalized1}\n`;
    }
    if (quantity2 > 0) {
